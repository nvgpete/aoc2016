...
int ok (struct silo p) {
  ...
  if ((fg0^=31)==0) return 1; // no gens, floor 1
  fc0^=31; 
...
int main(int argc,char **argv) {
  struct silo pnew,p={{0,30,0},{30,0,0},27,0}; // 27=theoretical fewest steps
  unsigned char flr,b,bb,c,g,cg,jj[5];
  int aptr,i,j;
  memset(fnd,255,sizeof(fnd));
  todo[0]=p;
  while ((todo[last-1].fc[2]&todo[last-1].fg[2])!=31) {
    if (last==0) exit(0);
    p=todo[--last];
    flr=0;
    ...
    c=(flr) ? p.fc[flr-1] : (p.fc[0]+p.fc[1]+p.fc[2])^31;
    g=(flr) ? p.fg[flr-1] : (p.fg[0]+p.fg[1]+p.fg[2])^31;
    cg=c&g;
    if (flr!=3) { // up first
